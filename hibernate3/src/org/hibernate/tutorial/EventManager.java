...
public class EventManager
{
	...
	private void addPersonToEvent2(Long personId, Long eventId)
	{
		...
		Event anEvent = (Event) session.load(Event.class, eventId);
		// If we want to handle it bidirectional and detached, we also need to
		// load this
		// collection with an eager outer-join fetch, this time with Criteria
		// and not HQL:
		/*
		 * Event anEvent = (Event) session
		 * .createCriteria(Event.class).setFetchMode("participants",
		 * FetchMode.JOIN) .add( Expression.eq("id", eventId) ) .uniqueResult();
		 * // Eager fetch the colleciton so we can use it detached
		 */
